...
(以上例子，完全是一个标准的Spring boot应用程序). 在这个例子中我们使用@EnableEurekaClient注解，但只有Eureka可以使用@EnableDiscoveryClient注解. Configuration is required to locate the Eureka server. Example:
